  ...
  if (req.method === 'POST') {
    ...
    req.on('end', async function () {
      ...
      try {
        ...
        // ---- Auth gate: tools/list, initialize, and register are public; tools/call requires auth ----
        var isPublicMethod = (rpcMethod === 'tools/list' || rpcMethod === 'initialize' || (rpcMethod === 'tools/call' && params.name === 'register'));
        var authz = isPublicMethod ? { ok: true, user: null, public: true } : checkAuth(reqApiKey, reqProfile, reqClientIp);
        if (!authz.ok) {
          auditLog({
            ts: new Date().toISOString(),
            event: 'auth_denied',
